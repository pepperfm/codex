...
fn normalize_manual_prefix_newline(
    ...
    for prefix in MANUAL_PREFIXES {
        if let Some(rest) = trimmed.strip_prefix(prefix) {
            let insert_at = leading + prefix.len();
            if rest.starts_with(' ') {
                let mut updated = raw;
                if insert_at < updated.len() {
                    updated.replace_range(insert_at..insert_at + 1, "\n");
                }
                return (updated, text_elements);
            }
            if rest.is_empty() {
                let mut updated = raw;
                ...
                let text_elements = text_elements
                    ...
                    .map(|elem| {
                        elem.map_range(|range| ByteRange {
                            start: if range.start >= insert_at {
                                range.start.saturating_add(1)
                            } else {
                                range.start
                            },
                            end: if range.end >= insert_at {
